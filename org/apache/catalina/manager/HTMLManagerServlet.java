  ...
  extends ManagerServlet
{
    ...
    try
    {
      ...
      else
      {
        ...
        else
        {
          ...
          File file = new File(this.deployed, filename);
          if (file.exists())
          {
    ...
    throws IOException
  {
    ...
    for (String contextName : contextNames)
    {
      ...
      if (ctxt != null)
      {
        ...
        args = new Object[7];
        args[0] = ("<a href=\"" + URL_ENCODER.encode(new StringBuilder().append(contextPath).append("/").toString()) + "\">" + RequestUtil.filter(displayPath) + "</a>");
        ...
        if (manager == null) {
          ...
          args[11] = Integer.valueOf(ctxt.getManager().getMaxInactiveInterval() / 60);
        }
    ...
    throws ServletException, IOException
  {
    ...
    else if ("removeSessionAttribute".equals(action))
    {
      ...
      String outMessage = "Session did not contain any attribute named '" + name + "'";
  ...
  protected List<Session> getSessionsForName(ContextName cn, StringManager smClient)
  {
    if ((cn == null) || ((!cn.getPath().startsWith("/")) && (!cn.getPath().equals(""))))
    {
      ...
      throw new IllegalArgumentException(smClient.getString("managerServlet.invalidPath", new Object[] { RequestUtil.filter(path) }));
    }
    Context ctxt = (Context)this.host.findChild(cn.getName());
    if (null == ctxt) {
      throw new IllegalArgumentException(smClient.getString("managerServlet.noContext", new Object[] { RequestUtil.filter(cn.getDisplayName()) }));
    }
    Manager manager = ctxt.getManager();
    ...
    throws ServletException, IOException
  {
    ...
    if ((null != sortBy) && (!"".equals(sortBy.trim())))
    {
      ...
      if (comparator != null)
      {
        ...
        {
          req.setAttribute("error", "Can't sort session list: one session is invalidated");
        }
      }
      else
      {
    ...
    getServletContext().getRequestDispatcher("/WEB-INF/jsp/sessionsList.jsp").include(req, resp);
  }
    ...
    throws ServletException, IOException
  {
    ...
    getServletContext().getRequestDispatcher(resp.encodeURL("/WEB-INF/jsp/sessionDetail.jsp")).include(req, resp);
  }
...
/* Location:              D:\F\阿里云架构开发\taobao-tomcat-7.0.59\taobao-tomcat-7.0.59\lib\catalina.jar!\org\apache\catalina\manager\HTMLManagerServlet.class
